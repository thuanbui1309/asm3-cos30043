...
router.post('/courses/:id/lessons', authenticate, async (req, res, next) => {
  try {
    ...
    const result = await sql`
      INSERT INTO lessons (id, course_id, title, video_url, duration, order_index)
      VALUES (${id}, ${courseId}, ${title}, ${video_url || null}, ${duration || 0}, ${order_index || 0})
      RETURNING *
    `;
...
router.put('/lessons/:id', authenticate, async (req, res, next) => {
  try {
    ...
    if (lesson.rows[0].instructor_id !== req.user.id) {
